...
- **Что делает:** Система принимает транзакции от пользователей, проверяет их через DSL-движок фрауд-правил (например, `amount > 100000`, `channel == MOBILE`) и автоматически присваивает статус `APPROVED` или `DECLINED`.
- **Для кого:** Учебно-соревновательный проект для **PROD 2026 Backend Olympiad**.
- **Какие проблемы решает:** Демонстрирует навыки построения асинхронного REST API с аутентификацией, RBAC, пакетной обработкой данных и аналитикой.

---

## 🏷 Что это за проект?

- **Тип:** REST API / Backend-сервис
- **Формат:** Backend API на FastAPI с Swagger/OpenAPI документацией
- **Уровень:** Учебно-соревновательный проект / MVP реальной антифрод-системы
